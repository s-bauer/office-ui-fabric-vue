...
import { getDocument, setPortalAttribute } from "@s-bauer/uifabric-utilities";
...
export default class OfficeLayer extends Vue {
  ...
  private beforeMount() {
    if (this.hostId) registerLayer(this.hostId, this);
  }
  ...
  private beforeDestroy() {
    if (this.hostId) unregisterLayer(this.hostId, this);

    if (this.layerVue) this.layerVue!.$destroy();
    ...
    this.layerVue = new VueNS({
      ...
      render(createElement) {
        ...
        const content = !self.$slots.default
          ...
          : createElement("OfficeFabric", fabricOptions, self.$slots
              .default as any);

        return createElement("div", rootOptions, [content]);
      },
      components: {
        OfficeFabric: OfficeFabric as any
      }
    });
