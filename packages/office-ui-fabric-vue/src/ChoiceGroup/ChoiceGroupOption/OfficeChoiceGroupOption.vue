  ...
  <div :class="classNames.root">
    <div :class="classNames.choiceFieldWrapper">
      ...
      <label :for="id" :class="classNames.field">
        <span :id="labelId" class="ms-ChoiceFieldLabel">{{text}}</span>
      </label>
    </div>
  </div>
</template>
...
import { getStyles } from "./OfficeChoiceGroupOption.styles";
...
export default class OfficeChoiceGroupOption extends Vue {
  ...
  @Prop({ type: Object, default: null }) private imageSize?: {
    width: number;
    height: number;
  };
    ...
    return mergeStyleSets(
      getStyles({
        ...
        imageIsLarge:
          !!this.imageSrc &&
          !!this.imageSize &&
          (this.imageSize.width > 71 || this.imageSize.height > 71)
      })
    );
  }
}
