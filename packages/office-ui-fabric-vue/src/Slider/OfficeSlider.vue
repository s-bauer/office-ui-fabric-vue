...
export default class OfficeSlider extends Vue {
  ...
  private get thumbOffsetPercent(): number {
    return this.min === this.max
      ? 0
      : ((this.renderedValue - this.min) / (this.max - this.min)) * 100;
  }
  ...
  private created() {
    ...
    if (calcValue !== this.value) this.$emit("change", calcValue);
  }
  ...
  private onMouseMoveOrTouchMove(
    ...
    if (!this.$refs.sliderLine) return;

    const { max, min, step } = this;
    const steps: number = (max! - min!) / step!;
    const sliderPositionRect: ClientRect = (this.$refs
      .sliderLine as HTMLDivElement).getBoundingClientRect();
    const sliderLength: number = !this.vertical
  ...
  private updateValue(value: number, renderedValue: number): void {
    ...
    if (isFinite(this.step)) {
      while (
        Math.round(this.step * Math.pow(10, numDec)) / Math.pow(10, numDec) !==
        this.step
      ) {
        numDec++;
      }
